...
<div>
    ...
    <section class="relative flex h-[85vh] min-h-[560px] items-end overflow-hidden bg-neutral-900 text-white">
        @if ($heroBanner)
            <img src="{{ $heroBanner->image_url }}" alt="{{ $heroBanner->title }}" class="absolute inset-0 size-full object-cover" fetchpriority="high">
        @endif
    ...
    @if ($newArrivals->isNotEmpty())
        <section class="rtw-container py-16 lg:py-24">
            <div class="mb-10 flex items-end justify-between">
                ...
                <a href="{{ route('shop.index', ['sort' => 'newest']) }}" wire:navigate class="rtw-link-underline hidden text-xs font-medium tracking-widest uppercase sm:inline-block">View All</a>
            </div>
    ...
    @if ($featuredCollection)
        <section class="grid lg:grid-cols-2">
            <div class="relative aspect-[4/3] lg:aspect-auto">
                <img src="{{ $featuredCollection->image_url }}" alt="{{ $featuredCollection->name }}" class="absolute inset-0 size-full object-cover" loading="lazy">
            </div>
            <div class="flex flex-col items-start justify-center bg-neutral-50 px-8 py-16 lg:px-16">
                ...
                <a href="{{ route('collections.show', $featuredCollection) }}" wire:navigate class="rtw-btn-primary mt-8">Discover the Collection</a>
            </div>
        </section>
    @endif
        ...
        <section class="rtw-container py-16 lg:py-24">
            ...
            <div class="grid grid-cols-2 gap-4 lg:grid-cols-5">
                @foreach ($categories as $category)
                    <a href="{{ route('shop.index', ['category' => $category->slug]) }}" wire:navigate class="group relative block aspect-[3/4] overflow-hidden bg-neutral-100">
                        <img src="{{ $category->image_url }}" alt="{{ $category->name }}" loading="lazy" class="absolute inset-0 size-full object-cover transition-transform duration-500 group-hover:scale-105">
                        <div class="absolute inset-0 bg-black/10 transition-colors group-hover:bg-black/25"></div>
                        <span class="absolute bottom-4 left-4 text-sm font-medium tracking-widest text-white uppercase">{{ $category->name }}</span>
                    </a>
                @endforeach
            </div>
        </section>
    ...
    {{-- Campaign Banner --}}
    @if ($campaignBanner)
        <section class="relative flex h-[60vh] min-h-[420px] items-center justify-center overflow-hidden bg-neutral-900 text-center text-white">
            <img src="{{ $campaignBanner->image_url }}" alt="{{ $campaignBanner->title }}" class="absolute inset-0 size-full object-cover" loading="lazy">
            <div class="absolute inset-0 bg-black/40"></div>
            <div class="relative px-6">
                <h2 class="font-serif text-3xl lg:text-5xl">{{ $campaignBanner->title }}</h2>
                <p class="mx-auto mt-4 max-w-lg text-sm text-white/80 lg:text-base">{{ $campaignBanner->description }}</p>
    ...
    @if ($bestSellers->isNotEmpty())
        <section class="rtw-container py-16 lg:py-24">
            <div class="mb-10 flex items-end justify-between">
                ...
                <a href="{{ route('shop.index') }}" wire:navigate class="rtw-link-underline hidden text-xs font-medium tracking-widest uppercase sm:inline-block">View All</a>
            </div>
    ...
    @if ($about)
        <section class="rtw-container py-16 lg:py-24">
            <div class="mx-auto max-w-xl text-center">
                ...
                <p class="mt-5 text-sm leading-relaxed text-neutral-600">{{ \Illuminate\Support\Str::limit(strip_tags($about->content), 220) }}</p>
                <a href="{{ route('pages.about') }}" wire:navigate class="rtw-btn-secondary mt-8 inline-flex">Our Story</a>
            </div>
        </section>
    @endif
</div>
